...
\[F_n(x):=f_n(s_n x) = \cos^n(s_n x), \quad  -\frac{\pi}{2s_n} \le x \le \frac{\pi}{2s_n}.\]
\begin{exercise}
Bizonyítsuk be, hogy
\[F_n''(1) = 0 \quad \text{és}\quad  F_n(1)\to g(1) = 1/\sqrt{e}\].
\end{exercise}
...
\subsection{Maximummal rendelkező függvények nagy hatványai}
\begin{frame}[t]{}
\begin{columns}
  \begin{column}{0.6\textwidth}
    \begin{blocklem}
    ...
    \end{blocklem}
    \begin{exercise}
     Bizonyítsuk be, hogy az $F_n$ függvények értelmezési tartománya
  ...
  \begin{column}{0.4\textwidth}
    \begin{alertblock}{Az átskálázott függvények}
      \[F_n(x) = \cos^n \left( \arctan\left(\frac{1}{\sqrt{n-1}}\right) x\right)\]
    \end{alertblock}
    \begin{center}
